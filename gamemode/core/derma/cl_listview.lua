...
function PANEL:Init()
    ...
    self:SetHeaderHeight( ScreenScale( 12 ) )
    self:SetDataHeight( ScreenScale( 8 ) )
...
function PANEL:AddColumn( strName, iPosition )
    ...
    if ( self.m_bSortable ) then
        pColumn = vgui.Create( "ixListView_Column", self )
    else
        pColumn = vgui.Create( "ixListView_ColumnPlain", self )
    end
...
function PANEL:FixColumnsLayout()
    ...
    while ( Remainder != 0 ) do
        ...
        -- Total max width of all the columns is less than the width of the ixListView, abort!
...
function PANEL:AddLine( ... )
    ...
    local Line = vgui.Create( "ixListView_Line", self.pnlCanvas )
...
vgui.Register( "ixListView", PANEL, "DPanel" )
